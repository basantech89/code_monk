class puzzle(object):
    ...
    def transition(self, to):
        label, blankLocation = self.label, self.spot
        newBlankLabel = str(label[to])
        newLabel = ""
        for i in range(9):
            if i == to : newLabel += "0"
            elif i == blankLocation : newLabel += newBlankLabel
            else : newLabel += str(label[i])
...
def BFSWithGenerator(start, end, q = []):
    ...
    while len(q) != 0:
        tmpPath = q.pop(0)
        lastNode = tmpPath[-1]
        if lastNode.label == end.label : return tmpPath
...
def notInPath(node, path):
    for elt in path:
        if node.label == elt.label : return False
    return True
...
def printSoultion(path):
    for elt in path : printGrid(elt)

#path = BFSWithGenerator(test1, goal)
#print printSoultion(path)
