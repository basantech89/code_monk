...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        min = self.points[0].distance(other.points[0])
    ...
    def averageLinkageDist(self, other):
        ...
        for sp in self.points:
            for op in other.points:
                avgdst += sp.distance(op)
        return avgdst/float(len(self.points) * len(other.points))
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        for point in c1.points:
            points.append(point); seen.append(point)
        for point in c2.points:
            if point not in seen : points.append(point)
    ...
    def findClosest(self, linkage):
        ...
        seen, bestD = [], linkage(self.members[0], self.members[1])
        bestC = (self.members[0], self.members[1])
        for member in self.members:
            seen.append(member)
            for other in self.members:
                if other not in seen:
                    d = linkage(member, other)
                    if d < bestD : bestD, bestC = d, (member, other)
        return bestC
    def mergeOne(self, linkage):
        ...
        #for member in self.members : print member
        #print '(', c1, c2, ')', 'yo'
        return (c1, c2)
