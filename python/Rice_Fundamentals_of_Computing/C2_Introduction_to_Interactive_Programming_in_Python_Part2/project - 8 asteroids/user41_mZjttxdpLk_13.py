...
time = 0
started = False
rocks, missiles, explosions = set(), set(), set()
...
def draw(canvas):
    global time, lives, score, started, rocks, explosions
...
def shoot():
    ...
    a_missile.pos[0] = my_ship.pos[0] + (my_ship.radius * cos)
    a_missile.pos[1] = my_ship.pos[1] + (my_ship.radius * sin)
    a_missile.vel[0] = my_ship.vel[0] + cos * 6
    a_missile.vel[1] = my_ship.vel[1] + sin * 6
...
def group_collide(group, sprite):
    ...
    for g in list(group):
        if g.collide(sprite):
            group.remove(g)
            explosions.add(Sprite(g.pos, [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound))
            f = True                  
    return f		            
