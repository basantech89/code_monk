...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		int TRI_H = 110, CIR_H = 140, SQR_H = 170, TEXT = 210, SLO = 240, INT = 270, DEE = 300, CRS = 330;
		int R = 15;
		fill(255, 250, 240);
		rect(25, 50, 150, 350);
		...
		fill(179, 36, 0);
		triangle(45, TRI_H + 10, 50, TRI_H, 55, TRI_H + 10);
		fill(color(255, 255, 255));
		ellipse(50, CIR_H + 5, 15, 15);
		fill(color(255, 255, 255));
		rect(45, SQR_H, R, R);
		fill(255, 255, 0);
		ellipse(50, SLO, R, R);
		...
		ellipse(50, DEE, R, R);
		strokeWeight(1);
		line(50 - 5, CRS - 5, 50 + 5, CRS + 5);
		line(50 + 5, CRS - 5, 50 - 5, CRS + 5);
		
		fill(0, 0, 0);
		text("City Marker", 75, TRI_H + 5);
		text("Land Quake", 75, CIR_H + 5);
		text("Ocean Quake", 75, SQR_H + 5);
		text("size ~ magnitude", 45, TEXT);
		text("Shallow", 75, SLO);
		text("Intermediate", 75, INT);
		text("Deep", 75, DEE);
		text("Past Day", 75, CRS);
	}
	...
	private void printQuakes() 
	{
		...
		for (Marker cm : countryMarkers) {
			int numQuakes = 0;
			String cmountry = (String)cm.getProperty("name");
			for (Marker m : quakeMarkers) {
				String mcountry = null;
				EarthquakeMarker em = (EarthquakeMarker)m;
				if (em.isOnLand()) {
					mcountry = (String)m.getProperty("country");
					if (mcountry.equals(cmountry)) numQuakes += 1;
				}
			}
			quakes += numQuakes;
			if (numQuakes > 0)
				System.out.println(cmountry + ": " + numQuakes);
		}
		System.out.println("OCEAN QUAKES: " + (quakeMarkers.size() - quakes));
	}
