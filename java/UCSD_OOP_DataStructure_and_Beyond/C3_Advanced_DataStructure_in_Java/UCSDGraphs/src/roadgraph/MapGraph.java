...
public class MapGraph {
	 ...
	 * Given n cities with one Hometown and all pairwise distances, plan a tour starting
	 * and ending at Hometown that visits every city exactly once and has optimum distance.
	 *
	 * @param hometown The hometown location
	 * @return The list of intersections that form a tour starting and
	 *   ending at Hometown (including both start and goal).
	 */
	public List<GeographicPoint> greedy(GeographicPoint hometown) {
		if (hometown == null) {
			System.out.println("Hometown is null! No path exists.");
			return new LinkedList<>();
		}

		MapNode homeNode = graph.get(hometown);

		Map<GeographicPoint, GeographicPoint> parent = new HashMap<>();
		Double dist = greedySearch(homeNode, parent);
		...
		// reconstruct the path
		return constructPath(hometown, hometown, parent);
	}


	/**
	 * Helper method for greedy algorithm
	 * @return the distance of the tour
	 * */
	private double greedySearch(MapNode homeNode, Map<GeographicPoint, GeographicPoint> parent) {
		// Initialization
		PriorityQueue<MapNode> queue = new PriorityQueue();
		Set<MapNode> visited = new HashSet<>();
		int count = 0; // counter for removed nodes from the queue
		...
		queue.add(homeNode);
		homeNode.setPredicted(0.0);

		// unless queue is empty or we have visited all the nodes
		while (!queue.isEmpty() || visited.size() == numVertices) {
			...
			if (curr == homeNode)
				// if we've visited all the nodes
				if (visited.size() == numVertices)
					return curr.getPredicted();

			double minStreetLen = Double.POSITIVE_INFINITY;
			MapNode minLenNode = null;	// node with min street length
			for (MapNode child : curr.getNeighbours().keySet())
				// if haven't visited the node or
				// visited all the nodes and ready to go back to the start node
				if (!visited.contains(child) || (visited.size() == numVertices &&
						child.getLocation() == homeNode.getLocation())) {
	...
	/** Helper method for bfs, dijkstra and greedy algorithm
	 * to construct the path from goal node to start node
	 * @param start The starting location
	 * @param goal The goal location
	...
	private List<GeographicPoint> constructPath(GeographicPoint start, GeographicPoint goal,
		...
		while ((p.getX() != start.getX() || p.getY() != start.getY()) || path.size() <= 1) {
			p = parent.get(p);
			path.add(0, p);
		}
	...
	public static void main(String[] args)
	{
		...
		MapGraph simpletest3 = new MapGraph();
		System.out.print("DONE. \nLoading the map...");
		GraphLoader.loadRoadMap("data/testdata/simpletest3.map", simpletest3);
		System.out.println("DONE.");
		GeographicPoint testStart3 = new GeographicPoint(0.0, 0.0);
		//GeographicPoint testEnd3 = new GeographicPoint(0.0, 0.0);
		List<GeographicPoint> testroute3 = simpletest3.greedy(testStart3);
		System.out.println(testroute3);


		System.out.print("Making a new map...");
		MapGraph simpleTestMap4 = new MapGraph();
		System.out.print("DONE. \nLoading the map...");
		GraphLoader.loadRoadMap("data/testdata/simpletest4.map", simpleTestMap4);
		System.out.println("DONE.");
		GeographicPoint testMap4Start = new GeographicPoint(9.0, 12.0);
		GeographicPoint testMap4End = new GeographicPoint(12.0, 30.0);
		//List<GeographicPoint> testroute = firstMap.bfs(testStart, testEnd);
		//List<GeographicPoint> firstMapRoute = firstMap.dijkstra(firstMapStart, firstMapEnd);
		List<GeographicPoint> testMap4Route = simpleTestMap4.greedy(testMap4Start);
		System.out.println(testMap4Route);
