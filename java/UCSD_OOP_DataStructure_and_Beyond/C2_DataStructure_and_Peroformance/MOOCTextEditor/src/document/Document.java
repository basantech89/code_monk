...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		boolean isLastCharSyllable = false;
		for (int i = 0; i < word.length(); i++)
		{
			char ch = word.charAt(i);
			if (i == word.length() - 1 && ch == 'e' && syllables != 0)
				continue;

			else if (vowels.contains(Character.toString(ch)) && !isLastCharSyllable) {
				syllables += 1;
				isLastCharSyllable = true;
			}

			if (i != word.length() -1 &&
					consonants.contains(Character.toString(word.charAt(i + 1))))
				isLastCharSyllable = false;
		}
	    return syllables;
	}
